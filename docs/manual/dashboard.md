---
title: 'Dashboard'
hide_title: true
---

import ManualHeader from '@site/src/components/manual/ManualHeader';
import ListaCard from '@site/src/components/shared/ListaCard';

<ManualHeader
  titulo="Dashboard"
  resumo="Indicadores e métricas do desempenho comercial.">

O **Dashboard** é a área destinada à **visualização geral das informações** do usuário ou da empresa. Nessa seção, são apresentados **indicadores e métricas** que auxiliam no **acompanhamento do desempenho comercial**.

</ManualHeader>

<ListaCard
  titulo="Principais informações exibidas"
  items={[
    { titulo: 'Total de Pedidos (R$)' },
    { titulo: 'Total de Orçamentos (R$)' },
    { titulo: 'Mix Médio de Produtos' },
    { titulo: 'Novos Clientes Cadastrados' },
    { titulo: 'Resumo de Vendas' },
    { titulo: 'Vendas por Categorias' },
    { titulo: 'Estatísticas Gerais' },
    { titulo: 'Análise de Perdas' },
    { titulo: 'Informações sobre Clientes' },
  ]}
/>

A seguir, são apresentadas **imagens ilustrativas do Dashboard**:

![Acessando o VendaAI](https://i.imgur.com/Jrwa2zf.png) 

---

![Acessando o VendaAI](https://i.imgur.com/TdVufxw.png)

**Total de Pedidos (R$)** – Apresenta o **valor total**, em reais, de todos os **pedidos efetivados** no período selecionado.

**Total de Orçamentos (R$)** – Exibe o **somatório dos valores** de **orçamentos ainda não convertidos** em pedidos.

**Mix Médio de Produtos** – Indica a **média de produtos diferentes vendidos por pedido**, auxiliando na **análise da variedade de vendas**.

**Novos Clientes Cadastrados** – Mostra a **quantidade de novos cadastros** realizados no sistema dentro do **período filtrado**.

**Resumo de Vendas** – Gráfico de colunas que consolida as **principais informações de faturamento**, o **desempenho comercial** e comparativos entre períodos.

![Acessando o VendaAI](https://i.imgur.com/mXABDdv.png)

**Vendas por Categorias** – Gráfico em rosca que detalha as **vendas segmentadas por grupo ou categoria de produto**, permitindo identificar os **segmentos mais rentáveis**.

**Estatísticas em Geral** – Apresenta **dados agregados** sobre **volume de vendas, clientes e produtos**, oferecendo uma visão ampla do **desempenho da operação**.

![Acessando o VendaAI](https://i.imgur.com/HCbcDab.png)

**Análise de Perdas e Clientes** – Identifica **clientes inativos**, **pedidos cancelados** e **oportunidades perdidas**, contribuindo para ações de **recuperação e fidelização**.


