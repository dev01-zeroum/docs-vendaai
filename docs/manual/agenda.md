---
title: 'Agenda'
hide_title: true
---

import ManualHeader from '@site/src/components/manual/ManualHeader';
import ListaCard from '@site/src/components/shared/ListaCard';

<ManualHeader
  titulo="Agenda"
  resumo="Tarefas, compromissos e atividades da equipe comercial.">

Na **Agenda**, os usuários podem acessar **relatórios específicos de vendedores**, permitindo uma visão detalhada do **desempenho** e das **atividades realizadas**.

</ManualHeader>

A interface possibilita a visualização das **tarefas e compromissos**, com opções para alternar entre os modos:

<ListaCard>

- **Mês**
- **Semana**
- **Dia**

</ListaCard>

Esses modos facilitam a **organização da rotina** e o **acompanhamento das atividades comerciais**:

![Acessando o VendaAI](https://i.imgur.com/ve49aOh.png)

Na parte superior da tela, no canto superior direito, encontra-se o botão azul **“+ Atividade”**, que permite a **adição de compromissos diretamente no sistema**.

Além disso, os **filtros** e o **campo de pesquisa** possibilitam refinar as buscas e localizar **atividades específicas** com mais facilidade.

O layout apresenta uma **grade horária**, na qual os compromissos podem ser distribuídos ao longo do dia. Esse formato estruturado facilita o **planejamento**, o **acompanhamento das atividades** e proporciona maior **controle sobre as tarefas agendadas**.

### Principais funcionalidades da Agenda

<ListaCard>

1. **Acesse os Relatórios** – Consulte os **relatórios de vendedores** para obter uma visão detalhada das atividades e do desempenho comercial.
2. **Navegue pela Agenda** – Utilize as seções disponíveis para gerenciar **produtos, clientes, vendas e demais atividades comerciais**.
3. **Pesquise Atividades** – Utilize o campo de pesquisa para localizar **atividades específicas** na data desejada.
4. **Adicione Informações** – Inclua **notas** ou **descrições detalhadas** nas atividades, especificando horários e informações relevantes.
5. **Defina Metas** – Estabeleça **metas diárias ou semanais** para manter as atividades alinhadas aos objetivos comerciais.

</ListaCard>

