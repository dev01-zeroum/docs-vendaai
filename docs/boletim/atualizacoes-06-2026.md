...
![Apenas Visualização](https://i.imgur.com/VmzQAat.png)

_**Obs: A tag (Somente Visualização) é apenas um exemplo e não será exibida após habilitação do recurso**_

Ao habilitar essa opção, o campo permanece visível durante a criação, edição ou integração de pedidos, porém sem permitir alterações no valor informado. Essa funcionalidade é útil para preservar informações críticas, evitar mudanças indevidas e garantir maior controle sobre dados integrados ou previamente definidos.
...
![Integração com Frete Rápido e Intelipost](https://imgur.com/MteSxyi.png)

Após a criação do pedido, é possível selecionar a opção de frete rápido e realizar a cotação em seguida.

![Cotação de transportadoras](https://imgur.com/csiu1tZ.png)

Conforme demonstrado, são apresentadas diferentes opções de transportadoras, permitindo a escolha conforme a necessidade.
